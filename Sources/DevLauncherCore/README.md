...
- GitHub `RepositoryProvider` 把缓存读取、手动全量发现和每日选择性刷新分成独立入口。
  可解码缓存即使超过 7 天也能先展示；每日入口只通过注入的 `CommandRunner` 精确查询
  已勾选仓库，并按名称合并回快照，不改动未选条目。调度 Timer 留在 App，Core 只提供
  可由 `FakeClock` 检查的 24 小时到期计算与缓存契约。
